  ...
  methods: {
    ...
    initCharts() {
      getServerList().then(res => {
        ...
        const chartData = servers.map(server => parseFloat(server.mem_capacity)); // Get mem_capacity values
    ...
    initDateTimelineChart(purchaseData, expireData) {
      const dateTimelineChart = echarts.init(document.getElementById('date-timeline-chart'));
      const option = {
        title: {
          text: '服务器购买和过期日期',
          left: 'center'
    ...
    initManufacturerPieChart(data) {
      const manufacturerPieChart = echarts.init(document.getElementById('manufacturer-pie-chart'));
      const option = {
        title: {
          text: '服务器厂商分布',
          left: 'center'
